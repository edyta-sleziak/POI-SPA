...
export class LeafletMap {
  ...
  moveTo(zoom: number, latitude: number, longitude: number) {
    this.imap.setZoom(zoom);
    this.imap.panTo(new L.LatLng(latitude, longitude)); //todo sprawdz
  }

  zoomTo(latitude: number, longitude: number) {
    this.imap.setView(new L.LatLng(latitude, longitude), 8);
  }

  addMarker(latitude: number, longitude: number, popupText = '', layerTitle = 'default') {
